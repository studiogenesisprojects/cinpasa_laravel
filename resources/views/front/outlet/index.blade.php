    ...
    <div class="container">
        ...
        <div class="row mt-5">
                        ...
                        <div class="position-relative">
                            <div id="carouselExampleControls" class="carousel slide" data-ride="carousel" data-interval="false">
                                <a href="{{LaravelLocalization::getURLFromRouteNameTranslated(App::getLocale(), 'routes.products.showProduct', [
                                    "productCategory" => $higherDiscount[0]->categories[0],
                                    "product" => $higherDiscount[0]
            ...
            <div class="col-lg-6 mt-lg-0 mt-4">
                <div class="row">
                    @for($i = 1; $i < 5; $i++)
                        @if(isset($higherDiscount[$i]))
                            <div class="col-sm-6 {{$i % 2 != 0 ? 'mt-sm-0 mt-4' : ''}}">
                                <div class="border-card p-3">
                                    <div class="position-relative">
                                        <a href="#" title="Accede al artículo"><img class="w-100 border-img" src="{{ Storage::url($higherDiscount[$i]->getPrimaryImageUrlAttribute()) }}" alt="imagen outlet"></a>
                                        <div class="position-absolute transform-t-50 d-flex b-0 r-0 mr-3">
                                            <p class="btn-yellow mr-1">-{{$higherDiscount[$i]->discount}}%</p>
                                            <a href="#" title="Comparte el artículo"><img class="btn-products p-1 mr-1" src="{{ asset('front/img/icon-share.svg') }}" alt="icono compartir"></a>
                                            <a href="#" title="Añade a favoritos el artículo"><img class="btn-products p-1" src="{{ asset('front/img/icon-favorito.svg') }}" alt="icono favorito"></a>
                                        </div>
                                    </div>
                                    <a href="#" title="Accede al artículo">
                                        <p class="small color-blue mt-3">{{$higherDiscount[$i]->categories[0]->lang()->name}}</p>
                                    <p class="font-bold color-black">{{$higherDiscount[$i]->lang()->name}}</p>
                                    </a>
                                </div>
                            </div>
                        @endif
