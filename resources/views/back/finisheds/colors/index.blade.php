    ...
    <div class="content-box">
        <div class="element-wrapper">
            <h6 class="element-header">Colores ({{$colors->count()}})
                <a href="{{route('colores-acabados.create')}}" class="btn btn-primary float-right"><i class="ti-plus"></i> Nuevo Color</a>
            </h6>

            <div class="element-box">
                <h5 class="form-header">Colores de acabados</h5>
                <hr>
                                        ...
                                        <div class="btn-group">
                                            ...
                                            <div aria-labelledby="dropdownMenuButton2" class="dropdown-menu dropdown-menu-right">
                                                @if(Auth()->user()->role->canUpdate(App\Models\Section::find(config('app.enabled_sections.acabados'))))
                                                    <a href="{{ route('colores-acabados.edit', $color->id)}}" class="dropdown-item"><i class="ti-pencil"></i> Editar</a>
                                                @endif
                                                @if(Auth()->user()->role->canDelete(App\Models\Section::find(config('app.enabled_sections.acabados'))))
                                                    <a href="" class="dropdown-item delete-register" data-toggle="modal" data-target="#modal-delete" data-url="{{ route('colores-acabados.destroy', $color->id) }}"><i class="ti-trash"></i> Eliminar</a>
                                                @endif
                                            </div>
                                        </div>
                                    </td>
