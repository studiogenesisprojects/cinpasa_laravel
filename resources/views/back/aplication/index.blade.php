    ...
    <div class="content-box">
        <div class="element-wrapper">
            <h6 class="element-header">
                Aplicaciones ({{ $aplications->count() }})
                <a href="{{ route('aplicaciones.create') }}" class="btn btn-primary float-right"><i class="ti-plus"></i>
                    Nuevo</a>
            </h6>
            @if(session('messages'))
                ...
                <div class="table-responsive">
                    <table id="applications" width="100%" class="table table-striped table-lightfont table-hover">
                        ...
                        <tbody>
                            ...
                            <tr role="row">
                                <td>
                                    <span class="d-none">{{ $aplication->order }}</span>
                                    <input class="form-control order" product_id="{{ $aplication->id }}" type="number" value="{{ $aplication->order }}">
                                </td>

                                <td><strong>{{ $aplication->name }}</strong></td>
                                <td>
                                    <button  product_id="{{ $aplication->id }}" type="btn" class="toggle btn btn-small @if($aplication->active) btn-danger @else btn-success @endif ">@if($aplication->active) Desactivar @else Activar @endif</button>
                                </td>
                                <td>
                                    @foreach ($aplication->applicationCategories as $ac)
                                        <span class="tag">{{$ac->lang()->name}} </span>
                                    @endforeach
                                </td>
...
<script>
    ...
    $("#applications").on("click", ".toggle", function(e){
            ...
            axios.get('/admin/aplicaciones/toggle-active/'+ id)
                .then(r => {
                    if(r.data.active){
